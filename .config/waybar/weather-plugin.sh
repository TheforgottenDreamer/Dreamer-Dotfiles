...
APIKEY=`cat $HOME/.openWeatherKey`
...
function setIcons {
    ...
    if [ "$TEMP" -le $COLD_TEMP ]; then
        TEMP=" <span forground='$COLOR_COLD'></span> $TEMP $TEMP_ICON"
    elif [ `echo "$TEMP >= $HOT_TEMP" | bc` -eq 1 ]; then
        TEMP=" <span foreground='$COLOR_HOT'></span> $TEMP $TEMP_ICON"
    else
        TEMP=" <span foreground='$COLOR_NORMAL_TEMP'></span> $TEMP $TEMP_ICON"
    fi
}

function outputCompact {
    OUTPUT="$WIND <span foreground='$ICON_COLOR'>$ICON</span> $ERR_MSG $DESCRIPTION | $TEMP"
